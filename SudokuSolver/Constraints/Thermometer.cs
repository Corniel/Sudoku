...
/// </summary>
public sealed class Thermometer(ImmutableArray<Pos> path) : Constraint
{
    ...
    public sealed class Less(Pos appliesTo, Pos other, int delta) : Restriction
    {
        ...
        public Candidates Restrict(Cells cells) => Candidates.AtMost(cells[Other] - Delta);
    }

    [DebuggerDisplay("{AppliesTo} >= {Other} + {Delta}")]
    public sealed class More(Pos appliesTo, Pos other, int delta) : Restriction
    {
        public Pos AppliesTo { get; } = appliesTo;
        ...
        public Candidates Restrict(Cells cells) => Candidates.AtLeast(cells[Other] + Delta);
    }
}
