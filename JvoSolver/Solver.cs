...
public static class Solver
{
    ...
    public static Cells Solve(Clues clues)
    {
        ...
        foreach(var (pos, digit) in clues)
            Constrain(sudoku, pos, new(1U << (digit - 1)));
        ...
        for (Pos pos = Pos.O; pos < Variables; pos++)
        {
            solution[pos] = sudoku[pos].Digit;
        }
    ...
    private readonly record struct Variable(uint Options)
    {
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => 1 + BitOperations.TrailingZeroCount(Options);
        }
